...
class coursereminder_teacher extends \core\task\scheduled_task
{

    public function get_name()
    {
        return get_string('messageprovider:teacher', 'notificationtype_coursereminder');
    }
                ...
                foreach($entries as $course) {
                    ...
                    if($this->hasCompletionCriteria($course->id)) {
                        ...
                        foreach ($users as $user) {
                            ...
                            $notify = new coursereminder_notification($course, $user, 'teacher');
                            $notify->setUncompletedUsers($notCompletedUsers);
                            $notify->notify();
                        }
